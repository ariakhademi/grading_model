# streamlit to create an application
import streamlit as st
from sentence_transformers import SentenceTransformer, util
from sklearn.preprocessing import MinMaxScaler

# load pre-trained sentence transformer model
# this can change to improve accuracy
model = SentenceTransformer("all-MiniLM-L6-v2")

# title
st.title("Short Answer Grading Prototype for the ABA")
...
if st.button("Grade Answer"):
    ...
    if not ideal_answer.strip() or not user_answer.strip():
        ...
        if scaled_score > 4.0:
            ...
            st.info("Medium quality answer.")
        elif scaled_score > 2.0:
            st.warning("Not too bad, needs improvement.")
        else:
            st.error("Poor quality answer alert.")
